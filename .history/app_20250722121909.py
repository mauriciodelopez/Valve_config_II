...
# IMPORTANT: When deploying to PythonAnywhere, the API key is set in the WSGI file.
# We retrieve it here from the environment variables.
GEMINI_API_KEY = os.environ.get('GEMINI_API_KEY')

# Initialize model to None. It will be set if configuration is successful.
model = None

# Attempt to configure Gemini API and initialize the model
if not GEMINI_API_KEY:
    print("ERROR: GEMINI_API_KEY environment variable is not set. Please set it in your WSGI file or as an environment variable on PythonAnywhere.")
else:
    try:
        ...
        # Catch any exceptions during configuration or model initialization
        print(f"ERROR: Failed to configure Gemini API or initialize model: {e}")
        # Optionally, set GEMINI_API_KEY to None here to ensure the /recommend endpoint catches it
        GEMINI_API_KEY = None
...
def static_files(filename):
    """
    Serves static files (CSS, JavaScript, images) from the 'static' folder.
    This allows the browser to load linked resources like style.css, script.js, and your logo.
    """
...
def recommend_valve():
    """
    Handles POST requests from the frontend to get valve recommendations from the Gemini AI.
    It constructs a prompt based on form data, calls the Gemini API, and returns a JSON response.
    """
    # Ensure the model is initialized before attempting to use it
    # Check if 'model' object exists and is not None
    if model is None:
        print("Attempted to call /recommend but Gemini model was not initialized.")
        return jsonify({"recommendations": ["AI service is not configured. Please ensure your Gemini API key is correctly set on the server."], "suitable": False}), 500
    ...
    ai_prompt_text = data.get('aiPrompt', '') # Get the new AI prompt text
    ...
    - Additional Context/Requirements: {ai_prompt_text if ai_prompt_text else 'None provided'}
    ...
    try:
        ...
        try:
            ...
            suitable = ai_data.get('suitable', True)
            ...
            recommendations = ["AI response structure was unexpected. Please try different inputs.", ai_prompt_text] # Changed to ai_prompt_text for debugging
            suitable = False
...
if __name__ == '__main__':
    # This block runs when you run app.py directly (e.g., for local testing)
    # It will not run when PythonAnywhere serves your app via WSGI.
    # The GEMINI_API_KEY is retrieved from os.environ.get('GEMINI_API_KEY') at the top of the file.
    # For local testing, ensure you set GEMINI_API_KEY as an environment variable in your terminal
    # before running 'python app.py', e.g., export GEMINI_API_KEY='YOUR_LOCAL_API_KEY'
    if not GEMINI_API_KEY:
        print("WARNING: GEMINI_API_KEY is not set for local testing. Please set it as an environment variable.")
