...
if not GEMINI_API_KEY:
    # This message will appear in your PythonAnywhere error log if the key is missing
    ...
    # For now, we'll let genai.configure fail if the key is truly missing.
else:
    genai.configure(api_key=GEMINI_API_KEY)
    # Initialize the Gemini model only if the API key is present
...
def recommend_valve():
    ...
    if not GEMINI_API_KEY or 'model' not in globals():
...
if __name__ == '__main__':
    ...
    if GEMINI_API_KEY == 'AIzaSyB8X5zyMLjuLbyk-ggj510ROhuLjeekSoE' or not GEMINI_API_KEY: # Check if the placeholder or empty key is used
        print("WARNING: GEMINI_API_KEY is not set. Please set it in your WSGI file on PythonAnywhere or as an environment variable for local testing.")
        print("You can get a free API key from https://aistudio.google.com/app/apikey")
        # For local testing, you might want to uncomment the line below and put your key directly
        # os.environ['GEMINI_API_KEY'] = 'YOUR_LOCAL_TESTING_API_KEY' # ONLY FOR LOCAL TESTING, DO NOT COMMIT TO GIT
        # genai.configure(api_key=os.environ.get('GEMINI_API_KEY')) # Re-configure if key was just set
        # model = genai.GenerativeModel('gemini-2.0-flash') # Re-initialize model
